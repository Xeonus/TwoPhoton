...
/**
 * This class implements a Plugin used to segment neuronal somatas for a given image
 * 
 * The PlugIn accesses the gradient flow segmentation algorithm in the resources folder
 * 
 * @author Alexander van der Bourg, Brain Research Institute Zurich
 * @version 1.0
 */
public class Image_Segmentation implements PlugIn {

	// Fields for computation
		private ImagePlus sourceImg;
		...
		public void run(String arg) {
			...
			if (idList == null || idList.length < 1) {
				IJ.error("At least one image have to be opened!");
				return;
			}
			...
			gd.addChoice("Segmentation file: ", imgList, imgList[defaultImg1]);
			...
			sourceImg = WindowManager.getImage(idList[defaultImg1 = gd
					.getNextChoiceIndex()]);
			originalName = sourceImg.getShortTitle();
			...
			if (saveDir == null) {
				saveDir = sourceImg.getOriginalFileInfo().directory + "Segmentation\\";
